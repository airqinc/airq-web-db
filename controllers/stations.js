...
//GET - Devuelve una estacion por ID
router.get('/:id', function(req, res) {
	Station.findById(req.params.id, function(err, data) {
	    if(err) return res.status(500).send(err.message);
		res.status(200).jsonp(data);
	})
})
...
router.delete('/:id', function(req, res) {
	Station.delete(req.params.id, function(err, station) {
		...
		Zone.deleteStation(station.zone, station._id)

  		res.status(200).send(station._id);
	})
})
